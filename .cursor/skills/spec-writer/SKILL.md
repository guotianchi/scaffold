...
Run: `bash "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/get-config" artifacts.order` — generate in configured order (default: proposal → specs → design → tasks). Run with `artifacts.skip` — skip any listed artifacts.
...
```bash
node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_2_result "approved: <summary>"
node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_2_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
