...
Check workflow mode first: `node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" state get <change-dir> workflow`. If `tweak` → direct edit mode. If `hotfix` or `full` → standard contract-first discipline.

Config check: `bash "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/get-config" execution.inlineThreshold` (default: 3).

Branch/worktree preflight before ANY implementation edit (mandatory — do not skip):
1. Run the isolation check:
   ```bash
   node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" isolate <change-dir>
   ```
...
2. If `node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" isolate` exits non-zero: STOP. Do not edit `main`/`master` in place.
   Ask the user for explicit approval (and re-run with `node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" isolate <change-dir> --force`
...
### Per-Task Loop
1. **Dispatch implementer**: Use `/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/skills/build-executor/implementer-prompt.md` template. Extract task brief with `scripts/task-brief PLAN_FILE N`. Include: where task fits, brief path, interfaces from prior tasks, report file path.
...
```bash
node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" config --resolve-model <profile>
```
...
### Progress Ledger
Track in `.superpowers/sdd/progress.md`. Check for existing ledger — completed tasks are done. After each batch: `node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> batches_completed <N>`.
...
```bash
node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" checkpoint save <change-dir> \
...
DP-4 (execution mode): `node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_4_result "<mode>: <rationale>"` + timestamp.
DP-5 (debug escalation): `node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> dp_5_result "<resolution>"` + timestamp.
