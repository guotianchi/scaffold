...
### Conflict Detection
Run `node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" sync <change-dir>`. If conflicts are detected (same requirement modified by multiple changes), present the conflict list to the user for resolution order.
...
3. Record that merging is complete so the `executing → closing` guard allows closure:
   ```bash
   node "/Users/guotianchi/Documents/project/springboot-scaffold/.cursor/spec-superflow/scripts/spec-superflow.mjs" state set <change-dir> spec_merged true
   ```
